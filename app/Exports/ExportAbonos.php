...
class ExportAbonos implements FromCollection
{
    ...
    public function collection()
    {
        ...
        if ($IdCln < 0) {
            $Abonos = Abono::whereBetween('fecha_cuota_secc1', [$dtIni, $dtEnd])
                            ->orWhereBetween('fecha_cuota_secc2', [$dtIni, $dtEnd])->where('activo', 1)->get();
        } else {
            $Abonos = Abono::whereBetween('fecha_cuota_secc1', [$dtIni, $dtEnd])
            ->orWhereBetween('fecha_cuota_secc2', [$dtIni, $dtEnd])->where('activo', 1)->whereHas('credito', function ($query) use ($IdCln) {
